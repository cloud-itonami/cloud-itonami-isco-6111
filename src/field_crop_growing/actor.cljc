(ns field-crop-growing.actor
  "FieldCropGrowingActor — the ISCO-08 6111 independent field-crop-
  and-vegetable-growing actor as a `langgraph.graph/state-graph` (per
  ADR-2607011000 / CLAUDE.md Actors section). One graph run = one
  plant/harvest operation request (intake → advise → govern → decide
  ...
  The unconditional invariant: the FieldCropAdvisor can never
  directly commit a record or dispatch a robot action the
  FieldCropGrowingGovernor refuses — every commit-record! call is
  gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [field-crop-growing.advisor :as advisor]
            [field-crop-growing.governor :as governor]
            [field-crop-growing.store :as store]))

(defn build-graph
  "Build a compiled FieldCropGrowingActor graph. `store` implements
  `field-crop-growing.store/Store`. `advisor` implements
  `field-crop-growing.advisor/Advisor` (defaults to `mock-advisor`).
